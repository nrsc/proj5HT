...
cells = list.files("~/proj5HT/den/serotonin/output/")
cell_json = list.files("~/proj5HT/den/serotonin/output/", recursive = TRUE, full.names = TRUE, pattern = ".json")
merged_metadata = read.csv("~/proj5HT/den/serotonin/Data/merged_metadata.csv", row.names = NULL, check.names = FALSE)[, -1]
...
puffNA = puffCellsMD[which(is.na(puffCellsMD$predicted_subclass)),]
...
```{r}
output_cellsMD_fltr = output_cellsMD %>% 
...
puffCellsMD_fltr = puffCellsMD %>% 
  dplyr::filter(Species %in% c("Human", "M.nemestrina")) %>% 
  dplyr::filter(class_Corr %in% c("exc", "Glutamatergic")) %>% # %>% 
  dplyr::filter(Cortical_area %in% c("MCx", "TCx"))# %>% 

table(puffCellsMD$Species)
```
...
cells_for_analysis = unique(puffPro$cell_name)[!unique(puffPro$cell_name) %in% unique(output_cellsMD$cell_name)]

unique(output_cellsMD$cell_name)

tstCell = "H21.03.304.11.04.02.02"

tstCell %in% cells_for_analysis

tst = puffPro[puffPro$cell_name %in% cells_for_analysis,]
identical(unique(tst$cell_name), cells_for_analysis)

tst2 = MD[MD$cell_name %in% cells_for_analysis,]
...
unique(puffPro$cell_name)[unique(puffPro$cell_name) %in% unique(output_cellsMD$cell_name)]


check_for_analysis = puffCellsMD[!puffCellsMD$cell_name %in% output_cellsMD$cell_name,]





table(need_analysis$Species)

write.csv(need_analysis, "~/proj5HT/den/serotonin/Data/50sPuffProtocols_to_add_to_analysis.csv")
