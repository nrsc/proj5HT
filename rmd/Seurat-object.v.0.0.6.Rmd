...
mtgFolder = "~/proj5HT/den/GreatApes_Macaque_NCBI/"
patchseqFolder = "~/proj5HT/den/macaque/MTG/GreatApes_Macaque_NCBI_RSC-204-386_map_full/"
...
###
#write_feather(cMTG, "~/proj5HT/data-raw/L6L5L23_cMTG.feather")
#saveRDS(cMTG, "~/proj5HT/data-raw/L6L5L23_cMTG.rds")
#saveRDS(gene, "~/proj5HT/data-raw/10xgenes.rds")
```


```{r 10x Counts, eval = FALSE}
cMTG = readRDS("~/proj5HT/data-raw/L6L5L23_cMTG.rds")
#cMTG<- read_feather("~/proj5HT/data-raw/L6L5L23_cMTG.feather")


#gene = readRDS("~/proj5HT/data-raw/10xgenes.rds")
...
rownames(brain.metadata.cMTG) <- colnames(brain.cMTG)
  
## Construct data set lists
brain.cMTG <- CreateSeuratObject(counts = cMTG, meta.data = brain.metadata.cMTG, project = "5HTcMTG")

saveRDS(brain.cMTG, "~/proj5HT/data-raw/Seurat-brain_cMTG_counts-patch-log2.rds")
```


# Do a percentage feature set for an initial observation of the data.  
```{r}
brain.cMTG[["percent.mt"]] <- PercentageFeatureSet(brain.cMTG, pattern = "^MT*")
```
...
```{r, eval = TRUE}
VlnPlot(brain.cMTG, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
```

# Without Normalizing the data

```{r, fig.height=15, fig.width=12}
VlnPlot(brain.cMTG, features = HTRgenes12, pt.size = 0.2, ncol = 2, group.by = "subclass")
```
...
saveRDS(dPatch, "~/proj5HT/data-raw/dPatch_counts-patch-log2.rds")
saveRDS(sMD, "~/proj5HT/data-raw/sMD.rds")
...
saveRDS(brain, "~/proj5HT/data-raw/Seurat-brain_dPatch_counts-patch-log2.rds")
...
```{r}
#brain = readRDS("~/proj5HT/data-raw/Seurat-brain_dPatch_counts-patch-log2.rds")
