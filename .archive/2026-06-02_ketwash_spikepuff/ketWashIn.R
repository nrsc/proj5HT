...
ketWashIn = function(x){
  ...
  if(cell %in% KWI$cell_name){
    ...
    plot(tst$time, tst$percent_change, main = paste(cell, "ket-wash-in", srt$dfs$spikeTTL$selected_MD$predicted[1]),xlab = "time", ylab = "% Change")
    plot(spikeTTL$spike_puff_output$time, spikeTTL$spike_puff_output$percent_change, main = paste(cell, "no-ket"),xlab = "time", ylab = "% Change")
