#' analysis function for the ketaserin washin
#'
#' @param x
#'
#' @returns
...
DrugWashIn = function(x){
  ...
  KWI_cells = unique(mMD[which(mMD$Exp == "Ket-wash-in"),"cell_name"])
  WWI_cells = unique(mMD[which(mMD$Exp == "WAYwash"),"cell_name"])

  washin_cells = unlist(c(KWI_cells, WWI_cells))
  ...
  if(cell %in% washin_cells){
    ...
    iM0 = iMerged[which(iMerged$Rank == "testSpike"),]
    ...
    sweep_peaks = sapply(names(nwb$sweeps), function(s) {
      sweep = nwb$sweeps[[s]]
      # Drop NA values
      sweep = sweep[which(!is.na(sweep))]
      sr = nwb$sampling_rate[[s]]
    ...
    plot(tst$time, tst$percent_change, main = paste(cell, iM0$Exp, srt$dfs$spikeTTL$selected_MD$predicted[1]),xlab = "time", ylab = "% Change")
    plot(spikeTTL$spike_puff_output$time, spikeTTL$spike_puff_output$percent_change, main = paste(cell, "baseline"),xlab = "time", ylab = "% Change")
    ...
    write.csv(tst, file.path(srt$rd, paste0(srt$cell, "-srtDrugWash.csv")))

    #srt$dfs$DrugWash = out
